...
namespace cleaner
{
	CcleanerMFCDlg m_form;

	//Remove all temp files in temp directories
	void clean_files()
	{
		//Windows temp
		if (m_form.Chk_ClearWindowsTemp)
		{
		...
		if (m_form.Chk_EdgeCookies)
		{
			...
			if (m_form.Chk_ClearEdgeHistory)
			{
	...
	void cleanup()
	{
		m_form.CListBoxLog.AddString(L"Cleaning");
		Beep(200, 200);
		clean_files();
	}
}